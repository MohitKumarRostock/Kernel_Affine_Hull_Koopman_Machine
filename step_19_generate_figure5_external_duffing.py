#!/usr/bin/env python3
"""Generate manuscript Figure 5 from the default Duffing external-baseline diagnostic.

Figure 5 in the manuscript is:

    figures/fig_external_duffing.pdf
...
The tuned Table 10 Duffing values use C=10, omega=2; this figure uses the
Experiment 15 default Duffing abstraction, C=15, omega=12, matching the current
manuscript caption.

Place this script in the same directory as the experiment scripts and run:

    python step_19_generate_figure5_external_duffing.py

Outputs:
    figures/fig_external_duffing.pdf
    kahkm_figure5_external_duffing/fig_external_duffing.png
    kahkm_figure5_external_duffing/figure5_plot_data.csv
    kahkm_figure5_external_duffing/figure5_metadata.json
"""
...
def parse_args() -> CliArgs:
    parser = argparse.ArgumentParser(
        description="Generate manuscript Figure 5 from the default Duffing external-baseline diagnostic."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("kahkm_exp15_default_external_duffing"),
        help="Output directory used by Experiment 15 for this default Duffing figure.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("kahkm_figure5_external_duffing"),
...
def run_experiment(args: CliArgs) -> None:
    ...
    print("Running Experiment 15 Duffing default external-baseline diagnostic:")
...
def load_plot_rows(args: CliArgs) -> list[PlotRow]:
    ...
    for raw in _read_csv(path):
        if str(raw.get("system", "")).lower() != "duffing":
            continue
    ...
    if missing:
        raise ValueError(f"Missing expected Duffing external-baseline rows: {missing}")
...
def write_plot_data(rows: Sequence[PlotRow], args: CliArgs) -> Path:
    ...
    path = args.output_dir / "figure5_plot_data.csv"
...
def make_figure(rows: Sequence[PlotRow], args: CliArgs) -> tuple[Path, Path]:
    ...
    ax.set_title("Duffing external Koopman baselines")
    ...
    pdf_path = args.figure_dir / "fig_external_duffing.pdf"
    png_path = args.output_dir / "fig_external_duffing.png"
...
def write_metadata(args: CliArgs, data_path: Path, pdf_path: Path, png_path: Path) -> Path:
    metadata = {
        "figure": "Figure 5",
        "manuscript_file": "figures/fig_external_duffing.pdf",
        ...
        "note": "This figure uses Experiment 15 default Duffing KAHKM settings C=15, omega=12. Table 10 uses the tuned Duffing setting C=10, omega=2.",
        "settings": {
            "system": "duffing",
    ...
    path = args.output_dir / "figure5_metadata.json"
