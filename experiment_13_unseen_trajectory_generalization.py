"""Experiment 13: unseen-trajectory generalization for tuned KAHKM closure.

Purpose
-------
Previous experiments used a suffix of the same trajectory as the held-out set.
This experiment trains one KAHKM model per system on one or more complete
training trajectories and evaluates it on separate trajectories generated from
unseen random seeds / initial perturbations.
...
Run
---
python experiment_13_unseen_trajectory_generalization_pylance_clean.py

Fast smoke test:
python experiment_13_unseen_trajectory_generalization_pylance_clean.py --systems duffing --train-seeds 0 --test-seeds 100 --n-steps-train 700 --n-steps-test 500 --horizons 1 10 50
"""
...
def make_concatenated_training_data(
    ...
    *,
    ...
    for seed in train_seeds:
        X0, X1 = make_snapshots(system, n_steps=int(n_steps_train), dt=float(dt), seed=int(seed), vanderpol_mu=float(vanderpol_mu))
        X0_parts.append(X0)
        X1_parts.append(X1)
    return np.concatenate(X0_parts, axis=1).astype(np.float64, copy=False), np.concatenate(X1_parts, axis=1).astype(np.float64, copy=False)
...
def _summarize_test_rows(rows_in: Sequence[CsvRow]) -> list[CsvRow]:
    ...
    for row in rows_in:
        key = (str(row["system"]), int(row["horizon"]))
        groups.setdefault(key, []).append(row)

    out: list[CsvRow] = []
    for (system, horizon), rows in sorted(groups.items()):
...
def parse_args() -> ExperimentArgs:
    parser = argparse.ArgumentParser(description="Experiment 13: unseen-trajectory generalization for tuned KAHKM.")
    ...
    parser.add_argument("--output-dir", type=str, default="kahkm_experiment_13_outputs")
...
def main() -> None:
    ...
    for system in args.systems:
        ...
        print(
            f"\n[{system}] training on seeds={args.train_seeds}, testing on seeds={args.test_seeds}, "
        ...
        fit_rows.append(
            {
                "system": system,
                "n_train_trajectories": len(args.train_seeds),
    ...
    _write_csv(output_dir / "experiment_13_fit_summary.csv", fit_rows, fit_fields)
    _write_csv(output_dir / "experiment_13_test_raw_results.csv", test_rows, test_fields)
    _write_csv(output_dir / "experiment_13_test_summary.csv", summary_rows, summary_fields)

    metadata = asdict(args)
    metadata["system_configs"] = {name: asdict(cfg) for name, cfg in SYSTEM_CONFIGS.items()}
    metadata["purpose"] = "Train on complete trajectories from train_seeds; evaluate on complete unseen trajectories from test_seeds."
    metadata["total_seconds"] = float(time.time() - start_all)
    with (output_dir / "experiment_13_metadata.json").open("w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=2)

    print("\nDone. Wrote outputs to:")
    print(f"  {output_dir / 'experiment_13_fit_summary.csv'}")
    print(f"  {output_dir / 'experiment_13_test_raw_results.csv'}")
    print(f"  {output_dir / 'experiment_13_test_summary.csv'}")
