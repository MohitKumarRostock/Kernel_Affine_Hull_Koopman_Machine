"""Experiment 06: abstraction ablation for KAHKM on Duffing.

Purpose
-------
This experiment compares the KAHKM folding abstraction against simpler
state-space regime abstractions while keeping the closure estimator fixed.
The goal is to isolate dictionary/abstraction quality from estimator quality.
...
Run
---
python experiment_06_duffing_abstraction_ablation.py

Fast smoke test:
python experiment_06_duffing_abstraction_ablation.py --random-states 0 --n-steps 600 --horizons 1 2 5 10 20
"""
...
def run_experiment(args: ExperimentArgs) -> None:
    ...
    _write_csv(output_dir / "experiment_06_one_step_raw.csv", one_step_rows)
    _write_csv(output_dir / "experiment_06_one_step_summary.csv", one_step_summary)
    _write_csv(output_dir / "experiment_06_multistep_raw.csv", multistep_rows)
    _write_csv(output_dir / "experiment_06_multistep_summary.csv", multistep_summary)
    _write_csv(output_dir / "experiment_06_run_diagnostics.csv", run_rows)
    ...
    with (output_dir / "experiment_06_metadata.json").open("w", encoding="utf-8") as handle:
        json.dump(metadata, handle, indent=2)

    print("\nWrote outputs to:")
    for name in [
        "experiment_06_one_step_summary.csv",
        "experiment_06_multistep_summary.csv",
        "experiment_06_run_diagnostics.csv",
        "experiment_06_metadata.json",
    ]:
...
def parse_args(argv: Sequence[str] | None = None) -> ExperimentArgs:
    parser = argparse.ArgumentParser(description="Experiment 06: abstraction ablation for KAHKM on Duffing.")
    ...
    parser.add_argument("--output-dir", type=str, default="kahkm_experiment_06_outputs")
