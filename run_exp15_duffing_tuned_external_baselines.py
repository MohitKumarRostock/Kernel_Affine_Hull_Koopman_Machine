#!/usr/bin/env python3
"""Run Experiment 15 external Koopman baselines with tuned Duffing KAHKM settings.

Purpose
-------
This runner is the next manuscript-upgrade step after the Duffing C/omega tuning.

It reruns:
    experiment_15_external_koopman_baselines.py

with the tuned Duffing KAHKM configuration:
    C = 10, omega = 2.0

instead of the older Duffing setting:
    C = 15, omega = 12.0

The original Experiment 15 file is NOT modified. A temporary patched copy is
...
Default run:
    python3 run_exp15_duffing_tuned_external_baselines.py

Dry run:
    python3 run_exp15_duffing_tuned_external_baselines.py --dry-run

Resume / skip existing output:
    python3 run_exp15_duffing_tuned_external_baselines.py --skip-existing

Outputs:
- kahkm_exp15_duffing_tuned_external_baselines/
- kahkm_exp15_duffing_tuned_external_baselines_results.zip

The output directory also contains:
- experiment_15_tuned_duffing_table_values.csv
- experiment_15_tuned_duffing_method_ranking.csv
...
def parse_args() -> RunnerConfig:
    parser = argparse.ArgumentParser(
        description="Run tuned Duffing Experiment 15 external Koopman baselines."
    )
    parser.add_argument("--python", default=sys.executable)
    parser.add_argument("--script", default="experiment_15_external_koopman_baselines.py")
    parser.add_argument("--output-dir", default="kahkm_exp15_duffing_tuned_external_baselines")
    parser.add_argument("--zip-name", default="kahkm_exp15_duffing_tuned_external_baselines_results")

    parser.add_argument("--systems", nargs="+", default=["duffing"])
...
def make_patched_script(
    *,
    ...
    # Make local imports work even though the patched script lives in output_dir.
    injection = (
        "import sys\n"
    ...
    old = (
        '"duffing": SystemConfig(name="duffing", dt=0.03, '
        "n_clusters=15, omega=12.0, tau=1e-6),"
    )
    new = (
        '"duffing": SystemConfig(name="duffing", dt=0.03, '
        f"n_clusters={int(duffing_c)}, omega={float(duffing_omega)!r}, tau=1e-6),"
    )

    if old in text:
        text = text.replace(old, new)
    else:
        # Fallback: replace the line containing the Duffing SystemConfig.
        lines = text.splitlines()
        replaced = False
        new_lines: list[str] = []
        for line in lines:
            if (
                '"duffing": SystemConfig(' in line
        ...
        if not replaced:
            raise RuntimeError(
                "Could not locate the Duffing SystemConfig line in Experiment 15."
            )
        text = "\n".join(new_lines) + "\n"

    patched_path = output_dir / "_patched_experiment_15_duffing_tuned.py"
...
def summarize_for_manuscript(output_dir: Path) -> None:
    ...
    _write_csv(output_dir / "experiment_15_tuned_duffing_table_values.csv", table_rows)
    ...
    _write_csv(output_dir / "experiment_15_tuned_duffing_method_ranking.csv", ranking_rows)
    ...
    _write_csv(output_dir / "experiment_15_tuned_duffing_fit_table.csv", fit_summary)
...
def main() -> None:
    ...
    patched_script = make_patched_script(
        ...
        duffing_c=config.duffing_c,
        duffing_omega=config.duffing_omega,
    )

    command_config = RunnerConfig(
        **{**asdict(config), "script": str(source_script), "output_dir": str(output_dir)}
    )
    command = build_command(command_config, patched_script)
    ...
    runner_metadata = {
        "runner": "run_exp15_duffing_tuned_external_baselines.py",
        "purpose": (
            "Rerun external Koopman baselines with the tuned Duffing "
            "KAHKM configuration C=10, omega=2."
        ),
    ...
    with (output_dir / "experiment_15_tuned_duffing_runner_metadata.json").open("w", encoding="utf-8") as handle:
        json.dump(runner_metadata, handle, indent=2)

    zip_path = make_zip(output_dir, zip_name=config.zip_name)

    print("\nFinished tuned Duffing external-baseline run.")
    print(f"Output directory: {output_dir}")
    print(f"Table values: {output_dir / 'experiment_15_tuned_duffing_table_values.csv'}")
    print(f"Method ranking: {output_dir / 'experiment_15_tuned_duffing_method_ranking.csv'}")
