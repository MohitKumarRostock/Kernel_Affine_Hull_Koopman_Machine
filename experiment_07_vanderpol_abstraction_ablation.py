"""Experiment 07: cross-system abstraction ablation for KAHKM on Van der Pol.

Purpose
-------
This experiment repeats the abstraction ablation on a second nonlinear
dynamical system: the Van der Pol oscillator. It checks whether the Duffing
findings are system-specific or transfer to another regime-structured flow.
...
Run
---
python experiment_07_vanderpol_abstraction_ablation.py

Fast smoke test:
python experiment_07_vanderpol_abstraction_ablation.py --random-states 0 --n-steps 600 --horizons 1 2 5 10 20
"""
...
def run_experiment(args: ExperimentArgs) -> None:
    ...
    _write_csv(output_dir / "experiment_07_one_step_raw.csv", one_step_rows)
    _write_csv(output_dir / "experiment_07_one_step_summary.csv", one_step_summary)
    _write_csv(output_dir / "experiment_07_multistep_raw.csv", multistep_rows)
    _write_csv(output_dir / "experiment_07_multistep_summary.csv", multistep_summary)
    _write_csv(output_dir / "experiment_07_run_diagnostics.csv", run_rows)

    metadata = asdict(args)
    metadata["total_seconds"] = float(time.time() - start_all)
    metadata["notes"] = (
        "Abstraction ablation: actual KAHKM folding versus KMeans hard, "
        "bounded distance-normalized, and RBF-soft simplex associations. "
        "The closure estimator is NLMS for every abstraction."
    )
    with (output_dir / "experiment_07_metadata.json").open("w", encoding="utf-8") as handle:
        json.dump(metadata, handle, indent=2)

    print("\nWrote outputs to:")
    for name in [
        "experiment_07_one_step_summary.csv",
        "experiment_07_multistep_summary.csv",
        "experiment_07_run_diagnostics.csv",
        "experiment_07_metadata.json",
    ]:
...
def parse_args(argv: Sequence[str] | None = None) -> ExperimentArgs:
    parser = argparse.ArgumentParser(description="Experiment 07: cross-system abstraction ablation for KAHKM on Van der Pol.")
    ...
    parser.add_argument("--output-dir", type=str, default="kahkm_experiment_07_outputs")
